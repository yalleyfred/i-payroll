...
type detail = {
    name: string; job_title: string; email: string; date: Date; basic_wage: number;
    allowance: number; bonus: number; income_tax: number; bonus_tax: number; snnit_deduction: number;
    loan_deduction: number; total_deduction: number; net_salary: number;
}
...
export const makePayslip = async(employee: {name: string}) => {
    try {
            ...
            console.log(emp?.name);
                ...
                const earning: number = empPayroll!.basic_wage + empPayroll!.allowance + empPayroll!.bonus; 
                console.log(earning);
